...
function amyseden_scripts() {
    ...
    // Main theme CSS
    wp_enqueue_style('amyseden-main', get_template_directory_uri() . '/assets/css/main.css', array(), '1.8.0');

    // Main theme JS (deferred)
    wp_enqueue_script('amyseden-main', get_template_directory_uri() . '/assets/js/main.js', array(), '1.8.0', true);
...
function amyseden_icon($slug, $class = '') {
    ...
    // Brand logos — full color, rendered as self-contained SVG
    if ($slug === 'google-g') {
    ...
    if ($slug === 'bbb-shield') {
        // BBB A+ style torch-badge (simplified, trademark-safe)
        return '<svg class="' . esc_attr($class) . '" viewBox="0 0 24 24" aria-hidden="true">'
            . '<path fill="#005A9C" d="M12 2 3 5v6c0 5.25 3.85 9.95 9 11 5.15-1.05 9-5.75 9-11V5l-9-3z"/>'
            . '<text x="12" y="11.5" font-family="Arial, sans-serif" font-size="5.5" font-weight="700" fill="#ffffff" text-anchor="middle">BBB</text>'
            . '<text x="12" y="17.5" font-family="Arial, sans-serif" font-size="5.5" font-weight="700" fill="#ffffff" text-anchor="middle">A+</text>'
            . '</svg>';
    }
...
/**
 * Trust badges — Google rating + BBB rating.
 * Render anywhere with echo amyseden_trust_badges('compact') or ('stacked').
 *
 * @param string $variant 'compact' (inline row), 'stacked' (two-up card), 'footer' (small footer strip)
 */
function amyseden_trust_badges($variant = 'compact') {
    if (!get_theme_mod('amyseden_trust_enabled', true)) {
        return '';
    }

    $google_rating = get_theme_mod('amyseden_trust_google_rating', '4.4');
    $google_count  = get_theme_mod('amyseden_trust_google_count', '120+ reviews');
    $google_url    = get_theme_mod('amyseden_trust_google_url', 'https://g.co/kgs/amyseden');
    $google_show   = get_theme_mod('amyseden_trust_google_show', true);

    $bbb_rating = get_theme_mod('amyseden_trust_bbb_rating', 'A+');
    $bbb_label  = get_theme_mod('amyseden_trust_bbb_label', 'BBB Accredited');
    $bbb_url    = get_theme_mod('amyseden_trust_bbb_url', 'https://www.bbb.org/');
    $bbb_show   = get_theme_mod('amyseden_trust_bbb_show', true);

    if (!$google_show && !$bbb_show) return '';

    // Compute filled stars (out of 5) with partial-fill support
    $rating_num  = floatval($google_rating);
    $full_stars  = floor($rating_num);
    $has_half    = ($rating_num - $full_stars) >= 0.3 && ($rating_num - $full_stars) < 0.8;
    $extra_full  = ($rating_num - $full_stars) >= 0.8 ? 1 : 0;
    $filled      = $full_stars + $extra_full;

    ob_start();
    ?>
    <div class="trust-badges trust-badges--<?php echo esc_attr($variant); ?>">
        <?php if ($google_show) : ?>
            <a class="trust-badge trust-badge--google" href="<?php echo esc_url($google_url); ?>" target="_blank" rel="noopener" aria-label="Google rating: <?php echo esc_attr($google_rating); ?> out of 5">
                <span class="trust-badge__logo"><?php echo amyseden_icon('google-g'); ?></span>
                <span class="trust-badge__body">
                    <span class="trust-badge__rating"><?php echo esc_html($google_rating); ?></span>
                    <span class="trust-badge__stars" aria-hidden="true">
                        <?php for ($i = 1; $i <= 5; $i++) :
                            $cls = ($i <= $filled) ? 'is-filled' : (($has_half && $i === $filled + 1) ? 'is-half' : '');
                        ?>
                            <span class="trust-star <?php echo esc_attr($cls); ?>"><?php echo amyseden_icon('star'); ?></span>
                        <?php endfor; ?>
                    </span>
                    <span class="trust-badge__meta">Google · <?php echo esc_html($google_count); ?></span>
                </span>
            </a>
        <?php endif; ?>

        <?php if ($bbb_show) : ?>
            <a class="trust-badge trust-badge--bbb" href="<?php echo esc_url($bbb_url); ?>" target="_blank" rel="noopener" aria-label="BBB rating: <?php echo esc_attr($bbb_rating); ?>">
                <span class="trust-badge__logo"><?php echo amyseden_icon('bbb-shield'); ?></span>
                <span class="trust-badge__body">
                    <span class="trust-badge__rating trust-badge__rating--bbb"><?php echo esc_html($bbb_rating); ?></span>
                    <span class="trust-badge__meta"><?php echo esc_html($bbb_label); ?></span>
                </span>
            </a>
        <?php endif; ?>
    </div>
